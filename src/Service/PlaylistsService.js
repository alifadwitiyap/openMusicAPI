...
class PlaylistService {
    ...
    async getPlaylists({ id: ownerId }) {
        console.log("🪵4life -> file: PlaylistsService.js -> line 32 -> PlaylistService -> getPlaylists -> ownerId -> ", ownerId);
        const query = {
    ...
    async verifyPlaylistOwner({ id: playlistId }, { id: ownerId }) {
        ...
        if (!result.rows.length) {
            throw new responseError('Playlist tidak ditemukan gagal memverifikasi   ', 404);
        }
    ...
    async addActivities({ id: playlistId }, { songId }, { id: userId }, action) {
        ...
        const query = {
            text: 'INSERT INTO playlists_song_activities VALUES ($1,$2,$3,$4,$5) RETURNING id',
