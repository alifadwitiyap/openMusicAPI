module.exports =
    class playlistHandler {
        ...
        async deletePlaylist(request, h) {
            await this._service.verifyPlaylistOwner(request.params, request.auth.credentials)
        ...
        async postAddPlaylistSong(request, h) {
            ...
            await this._service.verifyPlaylistAccess(request.params, request.auth.credentials)
            await this._service.addSongToPlaylist(request.params, request.payload)
            await this._service.addActivities(request.params, request.payload, request.auth.credentials, 'add')
        ...
        async getPlaylistSongs(request, h) {
            await this._service.verifyPlaylistAccess(request.params, request.auth.credentials)
            const playlist = await this._service.getPlaylistSongs(request.params, request.auth.credentials)
        ...
        async deletePlaylistSong(request, h) {
            ...
            await this._service.addActivities(request.params, request.payload, request.auth.credentials, 'delete')
